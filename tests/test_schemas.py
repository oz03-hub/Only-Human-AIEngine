...
def minimal_payload():
    ...
    return {
        "payload": {
            "groups_metadata": [
                {"group_id": 4, "status": "active", "status_updated_at": "2026-02-20T10:00:00Z"},
            ...
            "groups": [
                {
                    ...
                    "members": [
                        {"user_id": "e1000000-e5ef-4758-a0e3-e19a009b2853", "first_name": "Cristina", "last_name": None},
                        {"user_id": "50000000-780a-473c-91fc-b0043688eefc", "first_name": "Michelle", "last_name": None},
                    ],
                    "threads": [
                        {
                            "question": {
...
class TestWebhookSchemaParses:
    ...
    def test_minimal_payload_parses(self, minimal_payload):
    ...
    def test_groups_count(self, minimal_payload):
        req = WebhookIncomingRequest.model_validate(minimal_payload)
        assert len(req.payload.groups) == 1

    def test_group_threads_count(self, minimal_payload):
        req = WebhookIncomingRequest.model_validate(minimal_payload)
        assert len(req.payload.groups[0].threads) == 2

    def test_thread_messages_count(self, minimal_payload):
        req = WebhookIncomingRequest.model_validate(minimal_payload)
        assert len(req.payload.groups[0].threads[0].messages) == 2

    def test_empty_thread_messages(self, minimal_payload):
        req = WebhookIncomingRequest.model_validate(minimal_payload)
        assert req.payload.groups[0].threads[1].messages == []
...
class TestTimezoneNormalization:
    ...
    def test_short_negative_offset(self):
        msg = WebhookIncomingMessage.model_validate({
            "user_id": "abc",
            "content": "hello",
            "created_at": "2026-02-16 08:56:44+05",
            "is_ai": False,
        })
        assert isinstance(msg.created_at, datetime)
...
class TestOptionalFields:
    ...
    def test_thread_last_ai_message_at_optional(self):
        thread = WebhookIncomingThread.model_validate({
            "question": {
                "id": "q1", "text": "test", "options": [],
                "status": "active", "unlock_order": 1,
            },
            "messages": [],
        })
        assert thread.last_ai_message_at is None

    def test_metadata_status_updated_at_optional(self):
        meta = WebhookIncomingGroupMetadata.model_validate(
            {"group_id": 1, "status": "active"}
        )
        assert meta.status_updated_at is None
    ...
    def test_empty_group(self):
        payload = {
            "payload": {
                ...
                "groups": [
                    {"group_id": 4, "group_name": "Item 4 Chat", "members": [], "threads": []}
                ],
            }
        }
